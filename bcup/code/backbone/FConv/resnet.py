...
class ResNet(nn.Module):
    def __init__(self, block_en, num_blocks,num_class):
        ...
        self.classifier = nn.Conv2d(512,self.num_class,kernel_size=1,stride=1)
              
        # nn.Linear(512*block_en.expansion,self.num_class)
    ...
    def forward(self, x):
        ...
        prediction = F.relu(self.classifier(out))#(enc)
        print(prediction.size())
        prediction = F.avg_pool2d(prediction, prediction.size(2))
